...
The answer here is not “React rewritten in Koka”. The result is closer to a small Respo-style runtime with a React-like mental model plus some deliberate constraints:
...
Because listener identity is semantic, the framework now also carries framework-side protections in `respo/state.kk`:
